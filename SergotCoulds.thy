...
abbreviation tildeGconv  :: "i\<Rightarrow> (\<mu> set)\<Rightarrow>i\<Rightarrow>bool" ("_ -~-_  _")
  where "(\<tau>a -~- (G::\<mu> set) \<tau>b)  \<equiv> (\<tau>a \<^bold>~ \<tau>b)  \<and> (\<forall>x. ((x \<in> G) \<longrightarrow>  \<not> (~ x) (\<tau>a::i) (\<tau>b::i)))" 
...
abbreviation mstitconvDia   :: "\<mu> set \<Rightarrow> \<sigma>\<Rightarrow>\<sigma>" ("\<^bold><< _ \<^bold>>> _"[52]53)
  where "\<^bold><<(G::\<mu> set)\<^bold>>> \<phi> \<equiv> \<lambda>(w::i).\<exists>(v::i). (w -~- G  v) \<longrightarrow> \<phi>(v)"
...
abbreviation Could3 :: "\<mu> set \<Rightarrow> \<sigma> \<Rightarrow> \<sigma>" ("Could3\<^sub> _ _")
  where "Could3\<^sub> G \<phi>  \<equiv> \<^bold><<(G::\<mu> set)\<^bold>>> \<phi>"
