...
proof - 
  {assume asm1: "G \<noteq> {}"
    ...
    {assume red2: "\<not> (\<^bold>\<Delta>\<^sup>m\<^sup>a\<^sup>x\<^sub> G \<phi>) \<tau>" 
      ...
      {assume "G \<noteq> M"
        hence a: "M \<supset> G " using obtmax Collect_mono_iff asm3 mem_Collect_eq psubsetI by auto        hence "\<exists>x\<in>M. x\<notin>G" by auto
...
lemma singletonacc: "(\<tau> ~ {x}  \<tau>b) \<longleftrightarrow> (( ~ (x::\<mu>) \<tau>  \<tau>b))" 
  using xsub by auto
lemma eqclassalt: "y \<in> alt\<^sub> x (\<tau>) \<longrightarrow> ((~ (x::\<mu>)) (y::i)(z::i)) \<longrightarrow> z \<in> (alt\<^sub> x (\<tau>))" 
  by (metis mem_Collect_eq tildexeq)

lemma eqclassalt2: "cla \<in> (\<^bold>A\<^sub> x) \<longrightarrow> y \<in> cla \<longrightarrow> ((~ (x::\<mu>)) (y::i)(z::i)) \<longrightarrow> z \<in> cla" 
  using eqclassalt by blast

lemma eqclassalt3: "cla \<in> (\<^bold>A\<^sub> x) \<longrightarrow> y \<in> cla  \<longrightarrow> z \<in> cla \<longrightarrow> ((~ (x::\<mu>)) (y::i)(z::i))" using mem_Collect_eq tildexeq 
  by (smt (verit, del_insts))

lemma ac: "action \<in> (\<^bold>A\<^sub> x) \<longrightarrow> z \<in> action \<longrightarrow> y \<in> action \<longrightarrow> ( ~ x z y)" 
  using eqclassalt3 by blast
lemma rac: "action \<in> (\<^bold>A\<^sub> x) \<longrightarrow> z \<in> action \<longrightarrow> ( ~ x z y) \<longrightarrow> y \<in> action"
  using eqclassalt by blast
